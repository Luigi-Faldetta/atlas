...
import { getWatchlist, toggleWatchlist, getNote, saveNote } from '@/lib/localStorage'; // Added getNote, saveNote
...
export default function WatchlistPage() {
  ...
  const [currentTab, setCurrentTab] = useState<'properties' | 'notes'>('properties');
  ...
  const [activeDetailTab, setActiveDetailTab] = useState<'overview' | 'analytics' | 'ai'>('overview');
  ...
  const filteredAndSortedProperties = useMemo(() => {
    let filtered = watchlistedProperties.filter(prop =>
  ...
  const handleExport = () => {
    ...
    const headers = ['ID', 'Name', 'Location', 'Price', 'Yield', 'Appreciation', 'Score', 'Notes'];
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <Button variant="outline" onClick={handleExport} disabled={watchlistedProperties.length === 0}>
          <FileDown className="mr-2 h-4 w-4" />
          Export Watchlist
        </Button>
      </div>

      <Tabs value={currentTab} onValueChange={(value) => setCurrentTab(value as 'properties' | 'notes')} className="w-full">
        {/* Tabs List */}
        <TabsList className="mb-4">
          <TabsTrigger value="properties">Properties ({watchlistedProperties.length})</TabsTrigger>
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow mb-6 border border-slate-200 dark:border-slate-700">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-grow">
                <label htmlFor="search-watchlist" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Search</label>
                <Input
                  id="search-watchlist"
                  type="text"
              ...
              <div className="flex-shrink-0">
                <label htmlFor="sort-watchlist" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Sort By</label>
                <Select value={sortOption} onValueChange={(value) => setSortOption(value as SortOption)}>
                  <SelectTrigger id="sort-watchlist" className="w-full md:w-[180px]">
        ...
        <TabsContent value="properties">
          {filteredAndSortedProperties.length === 0 && watchlistedProperties.length > 0 ? (
        ...
        <TabsContent value="notes">
          {filteredAndSortedProperties.length > 0 ? (
            <div className="space-y-6">
              {filteredAndSortedProperties.map((property) => (
                <div key={property.id} className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow border border-slate-200 dark:border-slate-700">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <Button variant="outline" size="sm" onClick={() => handlePropertyClick(property)}>
                      View Property
                  ...
                  {editingNoteId === property.id ? (
                    // Note Editor
                    <div>
                      <Tabs defaultValue="edit" value={noteEditTab} onValueChange={(value) => setNoteEditTab(value as 'edit' | 'preview')}>
                        ...
                        <TabsContent value="preview">
                          <div className="prose dark:prose-invert max-w-none p-3 border rounded-md min-h-[100px] bg-slate-50 dark:bg-slate-700/50">
                             <ReactMarkdown>{currentNoteValue || '*No content yet*'}</ReactMarkdown>
                          </div>
                        </TabsContent>
                      </Tabs>
                       <div className="text-xs text-gray-500 dark:text-gray-400 mb-3">
                          Formatting Tips: Use **bold**, *italic*, # Heading, - item, `code`
                       </div>
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="sm" onClick={handleCancelEdit}>Cancel</Button>
                        <Button size="sm" onClick={() => handleSaveNote(property.id)}>Save Notes</Button>
                      </div>
                    </div>
                  ) : (
                    // Note Display
                    <div>
                      <div className="prose dark:prose-invert max-w-none mb-3 p-3 border rounded-md min-h-[50px] bg-slate-50 dark:bg-slate-700/50">
                        <ReactMarkdown>{notes[property.id] || '*No notes added yet*'}</ReactMarkdown>
                      </div>
                      <div className="flex justify-end">
                        <Button variant="secondary" size="sm" onClick={() => handleEditNoteClick(property.id)}>
         ...
         {watchlistedProperties.length === 0 && (
            <div className="text-center py-16 text-gray-500 dark:text-gray-400 border border-dashed border-gray-300 dark:border-gray-700 rounded-lg mt-6">
              ...
              <p className="mt-1 text-sm text-gray-500">Start adding properties to track your interests.</p>
              <div className="mt-6">
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent className="max-w-5xl max-h-[90vh] flex flex-col">
            ...
            <div className="border-b border-gray-200 dark:border-gray-700 mt-2">
                <Tabs value={activeDetailTab} onValueChange={(value) => setActiveDetailTab(value as 'overview' | 'analytics' | 'ai')} className="w-full">
                   <TabsList>
            ...
            <div className="flex-grow overflow-y-auto pr-3 -mr-3 pl-1 -ml-1 py-4">
              {activeDetailTab === 'overview' && <OverviewTab property={selectedProperty} aiTagsData={selectedPropertyTags} />}
              {activeDetailTab === 'analytics' && <AnalyticsTab propertyId={selectedProperty.id} history={selectedPropertyAnalyticsHistory} correlations={selectedPropertyAnalyticsCorrelations} liquidity={selectedPropertyAnalyticsLiquidity} />}
              {activeDetailTab === 'ai' && <AITab scoreData={selectedPropertyAI} tagsData={selectedPropertyTags} />}
            </div>
          </DialogContent>
        </Dialog>
      )}
